...
var UserActionsImpl = (function (_super) {
    ...
    function UserActionsImpl() {
        ...
        _this.renderDropDownItems = function () {
            ...
            _this.listItems.forEach(function (item, index) {
                list.push(<option value={item} key={index}>
                    {item}
                </option>);
            });
            return list;
    ...
    UserActionsImpl.prototype.render = function () {
        return (<div style={this.props.style || rightStyle}>
                <select value={this.props.action} onChange={this.saveAction} disabled={this.props.selectedIds.length === 0} style={this.props.selectedIds.length === 0 ? [disabledStyle, dropDownStyle] : dropDownStyle}>
                    {this.renderDropDownItems()}
                </select>
                <react_bootstrap_1.Button disabled={this.props.action === this.listItems[0] || this.props.selectedIds.length === 0} onClick={this.performAction} style={(this.props.action === this.listItems[0] || this.props.selectedIds.length === 0) ?
            disabledStyle : null}>
                    Go
