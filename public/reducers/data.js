...
function dataReducer(state, action) {
    ...
    var Model;
    switch (action.type) {
        case constants_2.FETCH_INSTANCE_DATA_START:
            return state;
        case constants_2.FETCH_INSTANCE_DATA_FULFILLED:
            ...
            if (action.payload) {
                ...
                throw new Error(constants_1.MISSING_ACTION_PAYLOAD);
            }
            return StoreService.setInstanceInList(state, instanceResource, new Model(instance), true);
        case constants_2.FETCH_INSTANCE_DATA_ERROR:
            return state.set(action.resource + "Edit", immutable_1.fromJS({
                hasError: true,
                isLoading: false,
            }));
        case constants_2.FETCH_INSTANCE_LIST_START:
            return state;
        case constants_2.FETCH_INSTANCE_LIST_FULFILLED:
                ...
                for (var payloadKey in payloadData) {
                    if (payloadData.hasOwnProperty(payloadKey)) {
                        if (payloadKey === instanceListKey) {
                            storeData.instanceList = payloadData[payloadKey].map(function (instanceKey) {
        ...
        case constants_2.FETCH_INSTANCE_LIST_ERROR:
